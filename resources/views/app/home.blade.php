...
@section('content')
    <div
        class="grid grid-cols-1 md:grid-cols-2 grid-rows-1 md:grid-rows-2 bg-white px-3 py-5 w-screen overflow-y-scroll mt-2 space-x-2 space-y-3">
                ...
                <div class="h-2/3 flex justify-center items-center">
                    @foreach ($ttArray as $trend)
                        <a id="slides" href="{{ route('article', topic($trend->topic, true)) }}"
                            ...
                            @else
                                <img width="300" height="300" src="{{ asset('images/logo_icon.png') }}"
                                    alt="">
                            @endif
        ...
        <div class="w-full rounded-sm max-h-fit">
            <div class="w-full border-b -skew-x-12 mb-1">
                <span class="uppercase text-sm relative left-2 bg-lime-500 text-white px-2 py-0.5 rounded-sm">Breaking
                    news</span>
            </div>
            <div class="p-1 space-y-1.5">
                @foreach ($breakings as $breaking)
                    <a href="{{ route('article', topic($breaking->topic, true)) }}"
                        class="flex flex-row justify-start items-start space-x-2">
        ...
        <div class="w-full rounded-sm max-h-fit">
            ...
            <div class="p-1 space-y-1.5">
                @foreach ($updates as $update)
                    <a href="{{ route('article', topic($update->topic, true)) }}"
                        class="flex flex-row justify-start items-start space-x-2">
                        @if ($update->hasImage())
