...
@section('content')
   <div class="flex flex-col justify-center items-center bg-white mx-2 my-4 rounded-md p-3">
        ...
        <div class="flex my-2 flex-row w-full justify-start items-center space-x-2">
            <img src="{{ asset('images/binkap.jpg') }}" alt="{{ $article->aut->username }}'s picture" class="rounded-full w-12 h-12">
            <div class="flex flex-col space-y-0 text-sm">
                <a href="{{ route('author.profile', $article->aut) }}" >by <span class="uppercase text-blue-400">{{ $article->aut->username }}</span></a>
                <span class="space-x-3 text-gray-500"><i class="fa fa-clock-o"> {{ $article->created_at->diffForHumans() }}</i><i class="fa fa-comments-o"> {{ $commentCount }} {{ Str::plural("Comment", $commentCount) }}</i></span>
            </div>
        </div>
        @if ($article->hasImage())
        ...
        @if ($commentCount > 0)
            <div class="flex my-5 justify-center items-center">
                <button id="comment-toggle" class="text-sm relative text-white bg-lime-500 px-2 py-0.5 uppercase -skew-x-12 rounded-sm w-fit cursor-pointer">
                    Show Comments
                </button>
            </div>
        ...
        @endif
        <button class="uppercase text-sm border border-gray-900 py-1 px-5 my-3 rounded-md text-gray-900 font-semibold" id="comment-form-toggle">
            Click to comment
        </button>
            ...
            <form action="{{ route('comment.create', $article) }}" method="post"    >
                ...
                @guest()
                    <div class="mb-2">
                        ...
                        class="w-full ring-1 ring-lime-500 rounded-md text-gray-800 p-1.5"
                        required
                        autocomplete="name"
                        @error('name')
                            autofocus
