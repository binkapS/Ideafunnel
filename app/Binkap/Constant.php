<?php

namespace App\Binkap;

class Constant
{
    public const ARTICLE_ID_LENGTH = 25;

    public const CATEGORY_ID_LENGTH = 15;

    public const COMMENT_ID_LENGTH = 20;

    public const USER_ID_LENGTH = 20;

    public const ARTICLE_TYPE_BREAKING = 100001;

    public const ARTICLE_TYPE_UPDATE = 100002;

    public const USER_STATUS_ADMIN = 100200;

    public const ARTICLE_STATUS_APPROVED = 100300;

    public const ARTICLE_STATUS_PENDING = 100301;

    public const ARTICLE_STATUS_REJECTED = 100302;

    public const ARTICLE_IMAGE_STORAGE_PATH = "blog/article/image";

    public const ADMIN_PROFILE_IMAGE_STORAGE_PATH = "blog/admin/image";

    public const CATEGORY_UNCATEGORISED = "hd37hff8fh4nf40";

    public const USER_BINKAP_S = "DfTs6gz8Mhz3qXrDmoSs";

    // todo
}
